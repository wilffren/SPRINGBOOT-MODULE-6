...
-- Relación ManyToOne: Un Event pertenece a un solo Venue
ALTER TABLE events 
...
-- Actualizar eventos existentes con venues
UPDATE events SET venue_id = 1 WHERE id = 1;  -- Teatro Nacional
UPDATE events SET venue_id = 1 WHERE id = 2;  -- Teatro Nacional
UPDATE events SET venue_id = 3 WHERE id = 3;  -- Centro de Convenciones
UPDATE events SET venue_id = 2 WHERE id = 4;  -- Auditorio Municipal
...
(7, 2);  -- Exposición de Arte -> Teatro (podría ser cultura)
